        ...
        switch (args[0].toLowerCase()) {
            ...
            case "give" -> {
                ...
                try {
                    ...
                    if (amount <= 0) {
                        sender.sendMessage("§cСумма должна быть > 0");
                        return true;
                    }
            ...
            // ✅ ИСПРАВЛЕНИЕ #1: /buyer reset теперь вызывает performReset()
            // которая обновляет и nextResetEpoch, и lastResetEpoch
            case "reset" -> {
                if (!sender.hasPermission("buyer.admin")) {
                    sender.sendMessage("§cНет прав!");
                    return true;
                ...
                data.performReset();
                sender.sendMessage("§a[Байер] §7Склад байера сброшен. Следующий сброс: §b"
                        + data.getNextResetTime().atZone(ZoneId.of("Europe/Kiev")).toLocalDateTime()
                        .format(java.time.format.DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")));
            }

            // /buyer info — информация о предмете и текущем складе
            case "info" -> {
                if (args.length < 2) {
                ...
                sender.sendMessage("  §7Цена покупки: §a"
                        + (item.getBuyPrice() == -1 ? "§cНельзя купить" : item.getBuyPrice()));
            ...
            // /buyer setstock — вручную задать склад
            case "setstock" -> {
                if (!sender.hasPermission("buyer.admin")) {
                    sender.sendMessage("§cНет прав!");
                    return true;
                ...
                if (args.length < 3) {
                    sender.sendMessage("§cИспользование: /buyer setstock <itemId> <amount>");
                    return true;
                }
                ShopItem item = ShopRegistry.getById(args[1]);
                if (item == null) {
                ...
                try {
                    ...
                    data.addStock(item.getId(),
                            amount - data.getServerStock(item.getId()), item.getServerLimit());
                    data.save();
