 ...
 *
 * FIX #1 — resetLimits():
 *   Убран fallback "initStock = serverLimit / 2".
 *   Теперь если initial-stock = 0 в items.yml — склад ставится в 0.
 *
 * FIX #2 — applyInitialStockForNewItems() переименован в applyInitialStockOnReload():
 *   При /buyer reload initial-stock из items.yml применяется для ВСЕХ предметов,
 *   а не только для новых (которых нет в data.yml).
 *   Старый метод applyInitialStockForNewItems() оставлен для первого старта
 *   (добавляет только предметы, которых ещё нет в data.yml).
 */
public class ShopData {
     ...
     * Вызывается ТОЛЬКО при первом старте / добавлении новых предметов.
     * Уже существующие предметы не трогаем.
     */
    ...
    // ─── FIX #2 ────────────────────────────────────────────────────────────
    /**
     * Применяет initial-stock из items.yml ко ВСЕМ предметам в реестре.
     * Вызывается при /buyer reload, чтобы изменения initial-stock сразу вступали
     * в силу для уже существующих предметов (не только для новых).
     */
    public void applyInitialStockOnReload() {
        for (ShopItem item : ShopRegistry.getAllItems()) {
            int initStock = ShopRegistry.getInitialStock(item.getId());
            serverStock.put(item.getId(), Math.min(initStock, item.getServerLimit()));
            log.info("[BuyerPlugin] Reload: склад '" + item.getId() + "' -> " + initStock);
        }
    }
    // ───────────────────────────────────────────────────────────────────────
    ...
    /**
     * Сброс склада (еженедельный сброс по таймеру или /buyer reset).
     *
     * FIX #1: убран fallback "if (initStock == 0) initStock = serverLimit / 2".
     * Теперь initial-stock: 0 в items.yml означает ровно 0 в складе после сброса.
     */
    public void resetLimits() {
        ...
        for (ShopItem item : ShopRegistry.getAllItems()) {
            int initStock = ShopRegistry.getInitialStock(item.getId());
            // ── FIX: убрана строка: if (initStock == 0) initStock = item.getServerLimit() / 2;
            serverStock.put(item.getId(), Math.min(initStock, item.getServerLimit()));
        }
    }
